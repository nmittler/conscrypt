...
public class ConscryptEngineTest {
    ...
    private SSLEngine newEngine(
        ...
        if (Conscrypt.Engines.isConscrypt(engine)) {
            setBufferAllocator(engine, bufferType.allocator);
            if (useAlpn) {
                Conscrypt.Engines.setAlpnProtocols(engine, SUPPORTED_ALPN_PROTOCOLS);
            }
        }
        return engine;
    }
    ...
    private static SSLContext newContext(Provider provider, TestKeyStore keyStore) {
        try {
            SSLContext ctx = SSLContext.getInstance(PROTOCOL_TLS_V1_2, provider);
